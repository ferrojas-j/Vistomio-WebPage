...
const PrivacyPolicy: React.FC = () => {
  ...
  const t = {
    es: ${JSON.stringify(es, null, 6).replace(/"([^"]+)":/g, '$1:')},
    en: ${JSON.stringify(en, null, 6).replace(/"([^"]+)":/g, '$1:')},
    fr: ${JSON.stringify(fr, null, 6).replace(/"([^"]+)":/g, '$1:')}
  };
...
fs.writeFileSync('src/PrivacyPolicy.tsx', newFileContent);
