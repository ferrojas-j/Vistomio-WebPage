    ...
    <div className="w-full max-w-[380px] mx-auto bg-slate-900 rounded-[3rem] p-3 shadow-2xl border-[6px] border-slate-950 relative shadow-black/30">
      ...
      <div className="w-full h-[675px] bg-slate-50 rounded-[2.5rem] overflow-hidden flex flex-col relative text-slate-800">
        ...
        <div className="pt-10 px-6 pb-4 flex justify-between items-center bg-white border-b border-slate-100 z-10 shadow-sm relative shrink-0">
          ...
          <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-gray-500 cursor-pointer">
            <X size={16} />
          </div>
        </div>
        ...
        <div className="flex-grow overflow-y-auto overflow-x-hidden bg-slate-50 pb-24 scrollbar-hide relative">
            ...
            <div className="p-4 animate-[fade-in_0.3s_ease-out]">
              ...
              <div className="flex justify-between items-center bg-white rounded-2xl p-4 shadow-sm mb-6 border border-slate-100">
                <div className="flex flex-col items-center gap-1">
                  <div className="w-3 h-3 rounded-full bg-boutique-gold"></div>
                  ...
                  <div key={idx} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex justify-between items-center cursor-pointer active:scale-95 transition-transform">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <div className={`text-[9px] font-bold uppercase tracking-wider
                        ${order.status === posDemo.orders.open ? 'text-boutique-gold' : 'text-gray-500'}
                      `}>{order.status}</div>
                    </div>
                  </div>
                ))}
          ...
          {activeTab === 'expenses' && (
            <div className="p-4 animate-[fade-in_0.3s_ease-out]">
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 mb-4 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-boutique-offwhite rounded-bl-[100px] -z-0"></div>
                <div className="relative z-10">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-gray-500 font-semibold tracking-wide">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
                  <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">{posDemo.expenses.initial}</div>
                  <div className="font-black text-slate-800 text-lg">$5,000.00</div>
                </div>
                <div className="bg-white rounded-2xl p-4 shadow-sm border-t-4 border-t-boutique-gold border-x border-b border-x-slate-100 border-b-slate-100">
                  <div className="text-[10px] font-semibold text-boutique-gold uppercase tracking-wider mb-1">{posDemo.expenses.current}</div>
                  <div className="font-black text-boutique-gold text-lg">$4,320.00</div>
                </div>
              </div>
          ...
          {activeTab === 'admin' && (
            <div className="p-4 animate-[fade-in_0.3s_ease-out]">
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 mb-4">
                ...
                <div className="flex items-center gap-1.5 text-[10px] text-gray-500 font-semibold tracking-wide uppercase">
                  <Shield size={14} className="text-gray-500" />
                  {posDemo.admin.shift}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
                  <div className="w-8 h-8 rounded-full bg-boutique-offwhite flex items-center justify-center mb-3">
                    <TrendingUp size={16} className="text-boutique-gold" />
                  </div>
                  <div className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">{posDemo.admin.totalSales}</div>
          ...
          {/* MODAL: CORTE DE CAJA */}
          {showCloseShift && (
            <div className="absolute inset-0 bg-boutique-offwhite/60 z-30 flex flex-col justify-end animate-[fade-in_0.2s_ease-out]">
              <div className="bg-slate-50 w-full h-[90%] rounded-t-[2.5rem] shadow-md flex flex-col overflow-hidden animate-[slide-up_0.3s_ease-out]">
                ...
                <div className="flex-grow overflow-y-auto p-4 scrollbar-hide bg-slate-50">
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-100 mb-4 overflow-hidden">
                    <div className="bg-slate-50/50 px-4 py-2 border-b border-slate-100">
                      <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">{posDemo.closeShiftModal?.income || 'INGRESOS DEL DÍA'}</span>
                    </div>
                    <div className="p-4 flex flex-col gap-3">
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-semibold text-slate-600">{posDemo.closeShiftModal?.cc || 'Tarjeta de crédito'}</span>
                        <span className="text-sm font-semibold text-slate-800">$8,500.00</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-semibold text-slate-600">{posDemo.closeShiftModal?.transfer || 'Transferencia'}</span>
                        <span className="text-sm font-semibold text-slate-800">$2,450.00</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-semibold text-slate-600">{posDemo.closeShiftModal?.cash || 'Efectivo'}</span>
                        <span className="text-sm font-semibold text-slate-800">$3,300.00</span>
                      </div>
                      <div className="h-px w-full bg-slate-100 my-1"></div>
                      <div className="flex justify-between items-center">
                        <span className="font-semibold text-slate-800">{posDemo.closeShiftModal?.totalIncome || 'Total Ingresos'}</span>
                        <span className="font-black text-boutique-gold">$14,250.00</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-100 mb-4 overflow-hidden">
                    <div className="bg-slate-50/50 px-4 py-2 border-b border-slate-100">
                      <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">{posDemo.closeShiftModal?.expenses || 'EGRESOS DEL DÍA'}</span>
                    </div>
                    <div className="p-4 flex flex-col gap-3">
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-semibold text-slate-600">{posDemo.closeShiftModal?.pettyCash || 'Gastos menores'}</span>
                        <span className="text-sm font-semibold text-slate-800">-$680.00</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-semibold text-slate-600">{posDemo.closeShiftModal?.providers || 'Proveedores'}</span>
                        <span className="text-sm font-semibold text-slate-800">-$1,200.00</span>
                      </div>
                      <div className="h-px w-full bg-slate-100 my-1"></div>
                      <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-3 mb-6">
                    <button className="flex-1 bg-white border border-boutique-sand text-slate-700 font-semibold py-3 rounded-xl flex items-center justify-center gap-2 active:bg-slate-50 transition-colors shadow-sm">
                      <Share2 size={16} />
                  ...
                  <button 
                    ...
                    className="w-full bg-transparent text-gray-500 font-semibold py-2 active:text-slate-700 transition-colors"
                  >
          ...
          {/* MODAL: NUEVO GASTO */}
          {showNewExpense && (
            <div className="absolute inset-0 bg-boutique-offwhite/60 z-30 flex flex-col justify-center px-4 animate-[fade-in_0.2s_ease-out]">
              <div className="bg-white w-full rounded-[2rem] shadow-md flex flex-col overflow-hidden animate-[scale-in_0.3s_ease-out]">
                
                <div className="p-6 relative">
                  <button onClick={() => setShowNewExpense(false)} className="absolute top-6 right-6 w-8 h-8 flex items-center justify-center text-gray-500 hover:text-slate-600 transition-colors cursor-pointer">
                    <X size={20} />
                  </button>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{tExpModal.amount || 'MONTO'}</label>
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 font-semibold">$</span>
                        <input type="text" placeholder="0.00" className="w-full bg-slate-50 border border-boutique-sand text-slate-800 font-semibold rounded-xl pl-8 pr-4 py-3 focus:outline-none focus:border-rose-300 focus:ring-1 focus:ring-rose-300 transition-all" />
                      </div>
                    </div>
                    
                    <div>
                      <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{tExpModal.category || 'CATEGORÍA'}</label>
                      <select className="w-full bg-slate-50 border border-boutique-sand text-slate-800 font-semibold rounded-xl px-4 py-3 focus:outline-none focus:border-rose-300 focus:ring-1 focus:ring-rose-300 transition-all appearance-none">
                    ...
                    <div>
                      <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{tExpModal.description || 'DESCRIPCIÓN'}</label>
                      <input type="text" placeholder={tExpModal.descPlaceholder || 'Ej: Verduras, Hielo, Detergente'} className="w-full bg-slate-50 border border-boutique-sand text-gray-500 font-medium rounded-xl px-4 py-3 focus:outline-none focus:border-rose-300 focus:ring-1 focus:ring-rose-300 transition-all" />
                    </div>
                  </div>
          ...
          {activeTable && (
            <div className="absolute inset-0 bg-white z-40 flex flex-col animate-[fade-in_0.2s_ease-out]">
              ...
              <div className="pt-10 px-4 pb-3 flex justify-between items-center bg-white border-b border-slate-100 shrink-0">
                ...
                <button onClick={closeTable} className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-gray-500 hover:bg-slate-100 transition-colors cursor-pointer">
                  <X size={16} />
                </button>
              </div>
                ...
                <div className="flex-grow overflow-y-auto flex flex-col pb-6 bg-slate-50">
                  ...
                  <div className="p-4 grid grid-cols-2 gap-3 shrink-0">
                    ...
                    {(menuItems[menuTab as keyof typeof menuItems] || []).length === 0 && (
                      <div className="col-span-2 py-8 text-center text-gray-500 text-sm font-medium">
                      ...
                      {cart.map((item, idx) => (
                        <div key={idx} className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm flex justify-between items-center animate-[fade-in_0.2s_ease-out]">
                          <div>
                            ...
                            <div className="text-xs text-gray-500 font-medium">${item.price} &times; {item.qty}</div>
                          </div>
                  ...
                  <div className="bg-white p-4 pt-4 border-t border-slate-100 mt-auto shrink-0 shadow-[0_-10px_20px_rgba(0,0,0,0.03)]">
                    <div className="flex justify-between items-end mb-4">
                      <span className="text-gray-500 font-medium">{tInt.total || 'Total'}</span>
                      <span className="text-3xl font-black text-boutique-gold">${cartTotal}</span>
                    </div>
                    <div className="flex flex-col gap-2">
                      <button 
                        onClick={() => {
                        ...
                        className={`w-full font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm ${cartTotal > 0 ? 'bg-boutique-gold hover:bg-boutique-gold text-white active:scale-95 shadow-boutique-gold/20 cursor-pointer' : 'bg-slate-100 text-gray-500'}`}
                      >
              ...
              {tableMode === 'checkout' && (
                <div className="flex-grow overflow-y-auto flex flex-col pb-6 bg-slate-50 relative">
                  ...
                  <div className="text-center py-8 bg-white border-b border-slate-100 shadow-sm shrink-0">
                    <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1">{tInt.toCharge || 'A COBRAR'}</div>
