    ...
    <div className="w-full h-full flex bg-white text-left text-boutique-navy font-sans">
      ...
      <div className="w-[280px] flex-shrink-0 bg-[#F4F1EA] flex flex-col overflow-hidden border-r border-[#E8DFD0]/50 h-full">
        ...
        <div className="p-8 flex items-center justify-center gap-3 mt-4 mb-4">
          <div className="w-14 h-14 rounded-[14px] border-2 border-[#C6A15B] bg-white flex items-center justify-center text-[#C6A15B] font-bold text-3xl shadow-sm flex-shrink-0">
            V
          </div>
        ...
        <div className="flex-1 py-4 space-y-2 overflow-y-auto scrollbar-hide [&::-webkit-scrollbar]:hidden">
          <MenuItem icon={<LayoutDashboard className="w-5 h-5" />} label={t.sidebar?.dashboard || 'Dashboard'} />
          <MenuItem icon={<CalendarDays className="w-5 h-5" />} label={t.sidebar?.bookingEngine || 'Reservas'} active />
          <MenuItem icon={<CreditCard className="w-5 h-5" />} label={t.sidebar?.payments || 'Pagos'} />
          <MenuItem icon={<UserCheck className="w-5 h-5" />} label={t.sidebar?.checkin || 'Check-in'} />
          <MenuItem icon={<Utensils className="w-5 h-5" />} label={t.sidebar?.restaurant || 'Restaurante'} />
          <MenuItem icon={<ClipboardList className="w-5 h-5" />} label={t.sidebar?.operations || 'Operaciones'} />
          <MenuItem icon={<DollarSign className="text-xl" />} label={t.sidebar?.finance || 'Finanzas'} />
      ...
      {/* --- MAIN CONTENT --- */}
      <div className="flex-1 flex flex-col bg-[#13203A] overflow-hidden relative p-6 gap-6">
        
        {/* CALENDAR WHITE CARD */}
        <div className="bg-white rounded-3xl flex-1 flex flex-col overflow-hidden shadow-xl min-h-0">
          {/* Top Header */}
          <div className="pt-6 pb-4 px-8 flex flex-row items-center justify-between bg-white flex-shrink-0 gap-0">
            <div className="flex items-center justify-start gap-6">
              <h1 className="text-2xl text-boutique-navy flex items-center gap-3 font-serif font-normal tracking-[0.03em] leading-relaxed">
                <CalendarDays className="w-6 h-6 text-[#C6A15B]" />
                Calendario
              </h1>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="bg-white border border-gray-200 rounded-full flex items-center shadow-sm">
                <button className="px-4 py-2 text-gray-400 hover:text-boutique-navy"><ChevronLeft className="w-4 h-4" strokeWidth={3} /></button>
                <div className="px-4 py-2 text-sm font-bold text-boutique-navy tracking-widest text-[#2A75D3]">HOY</div>
                <button className="px-4 py-2 text-gray-400 hover:text-boutique-navy"><ChevronRight className="w-4 h-4" strokeWidth={3} /></button>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-auto flex flex-col border-t border-gray-100">
            ...
            <div className="flex border-b border-gray-100 bg-white">
              <div className="w-48 flex-shrink-0 p-5 border-r border-gray-100 flex items-end">
              ...
              <div className="flex-1 grid grid-cols-7 relative">
                {['LUN 17', 'MAR 18', 'MIÉ 19', 'JUE 20', 'VIE 21', 'SÁB 22', 'DOM 23'].map((day: string, i: number) => (
                  <div key={i} className={`flex flex-col items-center justify-center py-4 border-r border-gray-100 ${i === 1 ? 'bg-[#FFF9E5]' : ''}`}>
                    ...
                    <span className={`text-xs mt-1 font-bold ${i === 1 ? 'text-[#C6A15B]' : 'text-[#86D097]'}`}>
            ...
            <div className="flex-1 flex flex-col overflow-y-auto scrollbar-hide [&::-webkit-scrollbar]:hidden">
              <div className="h-[56px] min-h-[56px] flex-shrink-0 relative flex border-b border-gray-100 group">
                <div className="w-48 flex-shrink-0 px-6 py-0 border-r border-gray-100 flex items-center">
                  <span className="text-[12px] font-bold text-boutique-navy/90 truncate">Suite Deluxe Terraza</span>
                </div>
                <div className="flex-1 grid grid-cols-7 relative">
              ...
              <div className="h-[56px] min-h-[56px] flex-shrink-0 relative flex border-b border-gray-100 group">
                <div className="w-48 flex-shrink-0 px-6 py-0 border-r border-gray-100 flex items-center">
                  <span className="text-[12px] font-bold text-boutique-navy/90 truncate">Suite Deluxe Jardín</span>
                </div>
                <div className="flex-1 grid grid-cols-7 relative">
                   {[...Array(7)].map((_, i) => <div key={i} className={`border-r border-gray-100 h-full ${i === 1 ? 'bg-[#FFF9E5]' : ''}`}></div>)}
              ...
              <div className="h-[56px] min-h-[56px] flex-shrink-0 relative flex border-b border-gray-100 group">
                <div className="w-48 flex-shrink-0 px-6 py-0 border-r border-gray-100 flex items-center">
                  <span className="text-[12px] font-bold text-boutique-navy/90 truncate">Habitación Deluxe 301</span>
                </div>
                <div className="flex-1 grid grid-cols-7 relative">
                   {[...Array(7)].map((_, i) => <div key={i} className={`border-r border-gray-100 h-full ${i === 1 ? 'bg-[#FFF9E5]' : ''}`}></div>)}
              ...
              <div className="h-[56px] min-h-[56px] flex-shrink-0 relative flex border-b border-gray-100 group">
                <div className="w-48 flex-shrink-0 px-6 py-0 border-r border-gray-100 flex items-center">
                  <span className="text-[12px] font-bold text-boutique-navy/90 truncate">Habitación Deluxe 302</span>
                </div>
                <div className="flex-1 grid grid-cols-7 relative">
                   {[...Array(7)].map((_, i) => <div key={i} className={`border-r border-gray-100 h-full ${i === 1 ? 'bg-[#FFF9E5]' : ''}`}></div>)}
              ...
              <div className="h-[56px] min-h-[56px] flex-shrink-0 relative flex border-b border-gray-100 group">
                <div className="w-48 flex-shrink-0 px-6 py-0 border-r border-gray-100 flex items-center">
                  <span className="text-[12px] font-bold text-boutique-navy/90 truncate">Habitación Jardín 201</span>
                </div>
                <div className="flex-1 grid grid-cols-7 relative">
                   {[...Array(7)].map((_, i) => <div key={i} className={`border-r border-gray-100 h-full ${i === 1 ? 'bg-[#FFF9E5]' : ''}`}></div>)}
              ...
              <div className="h-[56px] min-h-[56px] flex-shrink-0 relative flex border-b border-gray-100 group">
                <div className="w-48 flex-shrink-0 px-6 py-0 border-r border-gray-100 flex items-center">
                  <span className="text-[12px] font-bold text-boutique-navy/90 truncate">Habitación Jardín 202</span>
                </div>
                <div className="flex-1 grid grid-cols-7 relative">
                   {[...Array(7)].map((_, i) => <div key={i} className={`border-r border-gray-100 h-full ${i === 1 ? 'bg-[#FFF9E5]' : ''}`}></div>)}
        ...
        {/* CANALES CONECTADOS */}
        <div className="bg-white rounded-[24px] p-6 shadow-xl flex-shrink-0 flex flex-col justify-center overflow-hidden">
          ...
          <div className="flex gap-4 overflow-x-auto scrollbar-hide [&::-webkit-scrollbar]:hidden pb-2">
...
function BookingPill({ colStart, colSpan, color, name }: { colStart: number, colSpan: number, color: 'green' | 'blue' | 'gold', name: string }) {
  ...
  return (
    ...
    >
      <div className="flex w-5 h-5 rounded-full items-center justify-center flex-shrink-0 bg-black/10">
...
function ChannelCard({ name, color, subtitle, rooms }: { name: string, color: string, subtitle: string, rooms: string }) {
  ...
  return (
    <div className="flex flex-col justify-between border border-gray-200 rounded-[20px] p-4 bg-white flex-shrink-0 w-[180px] shadow-sm">
      ...
      <div className="flex items-end justify-between mt-auto">
        <div>
          <div className={`text-[11px] font-bold ${subtitle === 'Conectado' ? 'text-[#86D097]' : 'text-gray-400'}`}>{subtitle}</div>
          <div className="text-[10px] font-bold text-gray-500">{rooms}</div>
        </div>
        <div className={`w-2 h-2 rounded-full mb-1 ${subtitle === 'Conectado' ? 'bg-[#86D097]' : 'bg-[#C2AF8B]'}`}></div>
      </div>
    </div>
  );
}
